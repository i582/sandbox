...
import { Blockchain, BlockchainTransaction, type SandboxContract } from './Blockchain';
...
describe('Blockchain', () => {
    ...
    it('should have non-empty bounce bodies', async () => {
        ...
        expect(res.transactions).toHaveTransaction({
            from: address,
            body: beginCell().storeUint(0xffffffff, 32).storeSlice(body.beginParse()).endCell(),
        });
    });
    ...
    it('execution result in step by step mode should match one in regular mode', async () => {
        ...
        for (let i = 0; i < res.transactions.length; i++) {
            expect(
                compareTransaction(flattenTransaction(res.transactions[i]), flattenTransaction(stepByStepResults[i])),
            ).toBe(true);
        }
    });
    ...
    it('should return externals', async () => {
        ...
        await blockchain.setShardAccount(
            ...
            createShardAccount({
                address,
                code: Cell.fromBoc(
                    Buffer.from('te6ccgEBAgEAJgABFP8A9KQT9LzyyAsBAC7TXwRwVHAAc8jLAcsBywHLYcsfyXD7AA==', 'base64'),
                )[0],
                data: new Cell(),
                balance: toNano('1'),
            }),
    ...
    it('should trigger tick tock transaction', async () => {
        ...
        */
        ...
        if (res.description.type !== 'tick-tock') throw new Error('Tick tock transaction expected');
    ...
    it('should chain tick tock transaction output', async () => {
        ...
        if (tt.description.type !== 'tick-tock') throw new Error('Expected tick tock');
        expect(res.transactions[1].description.type).toEqual('generic');
